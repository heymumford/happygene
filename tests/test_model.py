...
class TestGeneNetwork:
    ...
    def test_gene_network_aggressive_scenario_target(self):
        """Aggressive scenario meets <5s target: 10k × 100 × 1000 < 5 seconds.

        This is the Phase 2 performance target. Validates that optimizations
        achieve desired throughput for typical use cases.

        Scenario: 10k individuals × 100 genes × 1000 generations
        ...
        # Benchmark: measure time to complete 1000 generations
        start = time.perf_counter()
        model.run(n_generations)
        elapsed_seconds = time.perf_counter() - start

        # Target: <5 seconds for Phase 2 performance goal
        target_seconds = 5.0
        throughput = (n_individuals * n_genes * n_generations) / elapsed_seconds
        throughput_million = throughput / 1_000_000

        print(f"\n{'Aggressive Scenario Results':^80}")
        print("=" * 80)
        print(f"Configuration:   {n_individuals} individuals × {n_genes} genes × {n_generations} generations")
        print(f"Total operations: {n_individuals * n_genes * n_generations:,}")
        print(f"Elapsed time:    {elapsed_seconds:.3f} seconds")
        print(f"Target time:     {target_seconds:.3f} seconds")
        print(f"Throughput:      {throughput_million:.2f} million ops/sec")
        print(f"Status:          {'PASS' if elapsed_seconds <= target_seconds else 'MISS'}")
        print("=" * 80)

        # Assert performance target
        assert elapsed_seconds <= target_seconds, (
            f"Aggressive scenario took {elapsed_seconds:.3f}s, "
            f"target was {target_seconds}s (overhead: {elapsed_seconds - target_seconds:.3f}s)"
        )
